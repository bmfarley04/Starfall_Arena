...
- Bug note: augment visuals that depend on shared runtime state should be replicated as state, not as a one-off particle spawn. Burner now syncs the burn application so remote copies can simulate the same burn ticks locally, while AutoCounter now syncs its active window from the server instead of letting each client free-run its own timer.
