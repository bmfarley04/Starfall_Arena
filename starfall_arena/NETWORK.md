...
- Bug note: augment visuals that depend on shared runtime state should be replicated as state, not as a one-off particle spawn. Burner now syncs the burn application and broadcasts server-authored burn tick visuals so remote copies match the authoritative timing, while Bubble Shield mirrors authoritative damage totals back into client runtimes every fixed update and anchor state is also broadcast explicitly on server-authoritative anchor transitions so client copies stay aligned.
