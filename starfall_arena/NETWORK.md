...
- Class5 charges are now server-authoritative. `NetMovement` ticks passive regen for client-owned Class5 copies, broadcasts the charge count/audio to all peers (so charge pips and charge gain/spend sounds show on remote proxies), and replays the four-shot primary fire audio burst for remote listeners instead of a single shot.
