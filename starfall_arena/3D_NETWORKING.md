...
- In networked Invasion, respawn/revive handling refreshes the resolved owner player's local presentation before HUD rebinds so the new ship's camera, input, and weapon aim-camera bindings are current before owner-side beam cosmetics are expected to render. If the selected beam weapon is already held when that refresh lands, the scene manager reasserts the owner-only cosmetic beam state so the client does not have to release and press fire again after a revive.
...
- high-frequency player/enemy movement snapshots are intentionally sent with unreliable RPC delivery. Reliable delivery queues stale movement states instead of dropping them, which can create permanent client-side jitter and even transport receive-queue warnings in Invasion when many enemies publish snapshots at the same time.
- `NetSnapshotInterpolator3D` normally samples against the observed NGO server tick, but it now falls back to the newest buffered snapshot tick when the observed server tick is far ahead of all received snapshot ticks. That mismatch means the sampling cursor is in a different tick range than the buffered data, so increasing visual delay alone cannot fix the starvation.
- `NetMovement3D` has gated owner-movement diagnostics for the current owned-ship jitter investigation. Enable `Enable Owner Movement Diagnostics` and keep `Log Owner Corrections` on to print `[NetMovement3D Correction]` entries with prediction-buffer status, tick context, input multipliers, movement config, Rigidbody before/after correction state, movement-lock/player-slot state, and recent combat/warp/boundary/dodge/reset side effects.
- Use `Correction Log Min Distance` to suppress harmless micro-corrections, `Correction Summary Interval Seconds` to rate-limit correction-rate summaries, `Log Movement Config On Spawn` to compare scene/prefab setup between 3dTest and 3D Invasion, and `Log Rigidbody State Drift` to include physics-state details in detailed correction logs.
- `NetDiagnosticsOverlay3D` is an optional local playtest overlay for movement networking. Add it to a scene object when diagnosing jitter; it reports snapshot buffer depth, adaptive delay, starvation, extrapolated frames, hard snaps, owner correction counts, correction rate, latest/largest correction distance, latest likely correction cause, and latest movement side effect for spawned 3D players/enemies. Player rows also report blocker sweep clamps (`clamp`, `clampRate`, `clampLatest`, `sweepR`, `blocker`); if corrections stay at zero but clamp rate rises continuously, the owned ship is being locally constrained by Invasion blocker sweeps rather than by server reconciliation.
- `InvasionSceneManager3D` logs a one-line spawned-player setup fingerprint after each prepared Invasion player spawn/respawn. Compare the logged slot, owner client id, reset status, reward/profile application state, and `NetMovement3D` flight-config fingerprint against the same prefab in 3dTest when owned-ship jitter appears immediately in wave 1.
- `NetInputSnapshot3D` carries the owner's max-speed multiplier in addition to thrust, rotation, and slow multipliers. `MovementSimulation3D` must apply that multiplier to precision-throttle speed caps and the final max-speed clamp so network prediction stays in parity with local `ShipFlight3D`.
