...
- Enemy beam pressure should be escapable through range control. Current enemy beam profile ranges were cut by roughly half so artillery, duelist, splitter, and Triumvirate beams create local danger zones instead of arena-scale tracking pressure. Beam engagement preferences are kept inside those actual beam ranges so enemies do not select beam attacks from distances where the beam cannot reach.
