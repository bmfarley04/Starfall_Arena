...
- Bug note: if a HUD canvas root is active but completely invisible in Scene/Game view, inspect its root `RectTransform.localScale` before chasing binding code. `heartCanvas` and `enemyCounterCanvas` both regressed with a root scale of `(0,0,0)`, which kept the manager references, active state, and text updates working while making the canvases impossible to see.
