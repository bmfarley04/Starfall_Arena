...
- `SiegeCarrierBossEnemyBrain3D` owns the second Invasion boss identity: a slow/stationary Siege Carrier that maintains a preferred range band without constantly rotating its hull toward the player, sequences one major bullet-hell-style pattern at a time, mixes targeted projectile pressure with lagging beam convergence, boss-centered orbital energy pillars, formation missile salvos, a two-hardpoint lightning slow beam, and optional enemy spawn waves, keeps a hard per-pattern projectile budget, and leaves authored escape lanes instead of flooding the whole arena
