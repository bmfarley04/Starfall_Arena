...
- `InvasionSceneManager3D` owns player lives and respawns for this mode: each player starts with the configured life count, loses one life on death, respawns at the death position while lives remain, and enters a zero-life rescue countdown if their teammate is still alive
- while the local player is in that zero-life rescue countdown and the teammate is still alive, the gameplay camera follows the surviving teammate; normal remaining-life respawns do not use the teammate camera, and the camera returns to the local owned player when the rescue resolves
- zero-life rescue revives spawn a fresh selected player ship near the surviving teammate after the configured countdown; the countdown is shown to the dead player through the reused `waveCanvas` text, pauses/resets if the teammate is not alive, and is resolved immediately on wave clear before reward offers are built so the revived player can choose an augment
- networked respawns and zero-life rescue revives reapply each slot's accumulated between-wave reward state on the newly spawned player instance; clients do this when the respawn-protection message resolves the fresh network player so local movement, weapons, HUD, and combat presentation keep the earned upgrades after death
...
- the old round-start countdown UI is not used; the reused `waveCanvas` text is also used for zero-life rescue countdowns
...
- the reused reward cards now keep the normal augment-select final-choice presentation: when a player confirms a reward, that chosen card still plays the white-flash/final-selection hold briefly before the reward UI closes and gameplay flow continues.
...
- developer wave skipping is available from `InvasionSceneManager3D`: when `Enable Developer Wave Skip Key` is on, the authoritative host can press the configured key, default `N`, to force the active wave through the normal wave-clear path. The wave manager stops pending spawns, clears/despawns currently tracked enemies, fires `WaveCleared`, grants the normal between-wave rewards on both peers through the existing reward network messages, and then continues to the next wave or victory flow.
...
- if both players run out of lives with no live teammate available to anchor a rescue, Invasion ends in defeat instead of leaving a dead team in an active wave
...
- while `InvasionSceneManager3D` is active, `Entity3D` rejects same-faction attacker damage at the final health mutation point; weapon/projectile/beam filters should still be authored correctly, but this protects Invasion from custom direct-damage bypasses
...
- `InvasionWaveManager3D.SpawnEnemyAt(...)` rejects live scene objects in networked sessions. Runtime-spawned NGO enemies must come from registered prefab assets so cloned enemies do not inherit dead/hidden/collider-disabled state from an existing object.
...
- `InvasionWaveManager3D` also owns arena-escape cleanup for alive enemies. On spawn authority, it periodically checks tracked, revealed enemies against the active `ArenaBoundary3D`; if an enemy is farther outside the arena than the configured `Arena Escape Kill Distance`, it kills the enemy through the normal enemy death/despawn path so wave progression, HUD enemy counts, and network clients stay in sync.
- `InvasionSceneManager3D` is the dedicated beginning-flow manager for networked Invasion. It owns player spawning, per-player life counts, death-position respawns, zero-life teammate rescue revives, rescue-wait teammate camera binding, respawn invulnerability presentation, gameplay HUD activation, wave text presentation, optional enemy counter presentation, optional heart/life counter presentation, UI canvas camera/sorting setup, and arena boundary startup.
...
- Bug note: player projectile targeting in Invasion must stay faction-driven even when no enemies are currently alive. GigaBlast could regress into duel targeting during pre-wave or between-wave gaps because the helper saw zero `EnemyTeam` entities and rewrote generic `"Enemy"` targeting to the teammate's `Player1`/`Player2` tag. Treat the active `InvasionSceneManager3D` as enough proof that generic player shots should remain `EnemyTeam`.
- Bug note: final damage rejection belongs at `Entity3D` too, not only inside weapons. Most Invasion combat paths are faction-filtered before calling `TakeDamage(...)`, but future custom direct-damage paths can miss that setup. Keep the Invasion-only same-faction damage guard in `Entity3D`, and keep non-damage combat effects such as Tractor Beam from affecting same-faction teammates separately.
...
- Bug note: Invasion-only visual tiers still use the shared `AugmentSelectManager` lookup paths. `tier4` is intentionally a single-card visual template, so wire only the first button/card references on the manager; missing the button, outer container, inner container, icon, title, or description can let the card appear while navigation, material highlights, or text/icon population still targets the wrong object. Reward offers must keep the active displayed row on tiers 1-3 and pass tier 4 through the per-card visual override list; activating the full tier 4 row draws the centered tier 4 card over the normal three-card offer instead of replacing a card slot.
...
- Bug note: CRAIZAN CONTRACT cards need the full tier 4 card object to replace their normal tier slot, not just copied image/text styling. A partial visual snapshot leaves the normal tier button/layout active underneath and cannot carry tier 4-specific child hierarchy, so the contract can look like it is overlaying a normal card. Mixed offers should clone the single tier 4 card template into the target slot at runtime, hide the normal slot button, and route selection/highlight lookups through the replacement.
...
- tune `Respawn Rules` on `InvasionSceneManager3D`: `Respawn Delay Seconds` controls how long the death presentation is allowed to breathe before the replacement ship appears, `Invulnerability Seconds` controls the temporary post-respawn damage immunity, and `Shield Flash Interval Seconds` controls how aggressively the manager pulses `ShieldController.OnHit(...)` for the visible high-alpha shield flash. `Rescue Revive` controls the zero-life teammate rescue path: enable/disable it, tune `Rescue Countdown Seconds`, set `Teammate Spawn Offset Distance`, and keep `Countdown Text Format` compatible with one numeric placeholder for the reused `waveCanvas` countdown.
...
- tune `Developer Controls` on `InvasionSceneManager3D` only for test builds/editor sessions. `Enable Developer Wave Skip Key` defaults on and `Developer Wave Skip Key` defaults to `N`; pressing it only works on the authoritative host/server and intentionally reuses the normal reward and next-wave flow instead of giving clients a separate debug-only state path.
...
- tune `Arena Escape Cleanup` on `InvasionWaveManager3D`: keep `Kill Enemies Far Outside Arena` enabled, set `Arena Escape Kill Distance` to the tolerated distance past the active boundary before cleanup kills an enemy, and keep the check interval relatively low-frequency because this is a rare failure recovery path rather than steering logic.
