...
- `SiegeCarrierBossEnemyBrain3D` owns the second Invasion boss identity: a slow/stationary Siege Carrier that maintains a preferred range band without constantly rotating its hull toward the player, sequences one major bullet-hell-style pattern at a time, mixes targeted projectile pressure with lagging beam convergence, boss-centered orbital energy pillars, formation missile salvos, and a two-hardpoint lightning slow beam, keeps a hard per-pattern projectile budget, and leaves authored escape lanes instead of flooding the whole arena
...
- add `SiegeCarrierBossEnemyBrain3D`; wire separate projectile weapon component arrays for lagging rake, predictive fan lanes, curtain lanes, and optional formation missile salvos, wire `BeamWeapon3D` components for the lagging beam convergence pattern, and wire exactly two lightning `BeamWeapon3D` components for the slow-beam pattern
...
- add either `Assets/Prefabs/3d_effects/SiegeCarrierOrbitalEnergyPillarVisual3D.prefab` for the preserved red/white V1 look or `Assets/Prefabs/3d_effects/SiegeCarrierBluePlasmaPillarVisual3D.prefab` for the blue/white V2 plasma look under the boss prefab, then assign that component to the brain's orbital pillar visual field; V2 uses `WhiteDwarfStar3D`, `Assets/Prefabs/3d_effects/SiegeCarrierBluePlasmaPillarBody3D.prefab`, blue plasma body materials, generated internal lightning ribbons, and spark billboards, while pillar behavior tuning still belongs on the `SiegeCarrierBossBalanceProfile3D` asset rather than only on the brain component
- create a `SiegeCarrierBossBalanceProfile3D` asset, assign it through `EnemyBalanceProfileApplier3D`, and tune the boss health, slow movement, preferred range min/max, plane bias, detection range, pattern cooldowns, shot budget, lane counts, orbital pillar count/radius/timings/damage, formation missile budget cost, beam telegraph, active duration, convergence lag, and beam aim smoothing there
