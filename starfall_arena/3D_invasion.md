...
- each reward asset now contains shared display data plus four hand-authored payload tiers: `Common`, `Epic`, `High`, and Invasion-only `Tier4`. Keep names/icons/descriptions shared unless a future task explicitly expands the presentation model. If a reward's `Tier4` payload is left empty, runtime selection falls back to that reward's `High` payload so existing reward assets remain functional until the fourth tier is tuned.
...
- tune `Between-Wave Rewards` on `InvasionSceneManager3D`: enable/disable the reward system, set how many cards appear per offer, tune `Reward Presentation Delay Seconds` for the pause after a wave clear before the reward cards appear, tune `Reward Post Presentation Delay Seconds` for the small pause after the reward screen closes and before the next-wave intro handoff, and either assign an explicit reward list in the inspector or maintain the shared default asset pool under `Assets/Resources/3D/InvasionRewards`.
- Invasion reward tiers cycle across cleared waves as `Common`, `Epic`, `High`, then single-card `Tier4`; with the current five-wave target, the fourth cleared wave offers the Invasion-only tier 4 reward before the final wave.
