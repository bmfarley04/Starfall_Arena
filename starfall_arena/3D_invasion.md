...
- `SiegeCarrierBossEnemyBrain3D` owns the second Invasion boss identity: a slow/stationary Siege Carrier that maintains a preferred range band without constantly rotating its hull toward the player, sequences one major bullet-hell-style pattern at a time, mixes targeted projectile pressure with lagging beam convergence, formation missile salvos, and a two-hardpoint lightning slow beam, keeps a hard per-pattern projectile budget, and leaves authored escape lanes instead of flooding the whole arena
...
- create a `SiegeCarrierBossBalanceProfile3D` asset, assign it through `EnemyBalanceProfileApplier3D`, and tune the boss health, slow movement, preferred range min/max, plane bias, detection range, pattern cooldowns, shot budget, lane counts, formation missile budget cost, beam telegraph, active duration, convergence lag, and beam aim smoothing there
