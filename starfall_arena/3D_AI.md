...
- `TriumvirateEnemyBrain3D`
  ...
  - `Anchor Formation Near Current Squad` is still constrained by the final beam's effective range; if the squad detects the player while too far away, it relocates toward `Formation Distance From Target` before linking so the final beam can actually reach
