...
- `AugmentSelectManager` supports an optional single-card fourth tier for reused 3D Invasion reward visuals. Tier 4 is intentionally optional and explicit-only: normal 2D duel scenes can leave the tier 4 pool, canvas group, button, containers, and text/icon references unassigned, and the normal random tier order stays limited to tiers 1-3. In mixed Invasion reward offers, tier 4 should be used as a per-card visual style on an existing tier 1-3 row rather than as the active displayed row.
...
- Bug note: reused Invasion reward cards should treat controller/keyboard selection as the only focus owner. Do not let pointer hover retake `_selectedCardIndex` at all, or a stale mouse position can keep snapping selection back to whichever card is under the cursor while the player is navigating with controller/keyboard. Mouse clicks may still activate a card directly, but hover should not own focus.
- Bug note: Invasion reward focus should be fully self-owned by `AugmentSelectManager`, not co-owned with Unity UI's selected-object lifecycle. If the reward screen still depends on `Select` / `Deselect` events or `EventSystem.currentSelectedGameObject` to drive card state, random EventSystem reselection can overwrite the manager's chosen card before the next manual navigation input and make the picker appear to "snap" between cards.
...
- Bug note: mixed Invasion offers with CRAIZAN CONTRACTS must replace the normal card slot with a runtime clone of the single tier 4 card template. Copying only border/icon/text visual properties onto the normal tier card leaves the normal card's layout and hierarchy in charge, which can make the contract appear partially overlaid instead of using its full authored setup.
